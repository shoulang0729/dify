...
/*
 * W8 が捕まえるもの／捕まえないもの（PR #230 で表記ゆれ・隣接の崩しを吸収した後の状態。
 * data/world/README.md「tools/check-world.mjs の読み方」の同名節から参照される。
 * この節は、強化後もなお 2 回すり抜けられたことを踏まえて追記した）:
 *
 * 捕まえる：
 *   - kpi.csv の全行（name_ja/name_zh。ハードコードしない）のラベルが本文に出ているのに、
 *     直後（隣接の崩し込みで最大 6 文字以内）の数値が基準値（target/current/prev）の
 *     どれとも一致しないケース
 *   - ラベルの表記ゆれ（働／動／动。KPI_LABEL_CONFUSABLES に列挙した組のみ）
 *   - ラベルと数値の間の「隣接の崩し」：コロン（全角/半角）・読点・カンマ・開き括弧
 *     （全角/半角）・強調のアスタリスク・助詞「は/の/が」・全角空白・半角空白、
 *     合計で最大 6 文字まで（KPI_ADJACENCY_RE）。半角空白は \s に含まれるので、
 *     本物の改行文字（U+000A）が 1〜数文字挟まる場合もここに含まれて捕まる
 *     （vm 評価後の mock/js/data 配下のテキストはこの経路。実測で確認済み）
 *
 * 捕まえない（reviewer が PR #230 マージ後に一時ファイルで実測。6 パターンとも
 * data/world/README.md に転記。本 PR は文書のみでこのロジックは変更していない）：
 *   - 全角数字＋全角％（例: 稼働率８６．０％）— 半角の [0-9] と半角 % しか受け付けない
 *   - 小数表記（例: 稼働率 0.86）— kpi.csv 側の基準値表記（86.5 等）と文字列として
 *     一致しない別表記
 *   - 「パーセント」表記（例: 稼働率 86 パーセント）— unit 列の記号（% 等）としか
 *     突き合わせない。仮名の単位表記は対象外
 *   - ラベルの言い換え（例: 設備の稼働は 86.0%）— kpi.csv の name_ja/name_zh との
 *     文字列一致が前提で、同義語・言い換えは追わない
 *   - 隣接 7 文字以上（例: 稼働率-------86.0%）— KPI_ADJACENCY_RE の上限 {0,6} を超える
 *   - 改行を挟む場合の一部：dify/tests/**.json・dify/kb/**.md 等「生テキストとして
 *     読み JSON.parse 等の解釈をしない」対象で、文字列中の改行がエスケープ表現
 *     （ソース上はバックスラッシュ＋n の 2 文字）のまま残っているケース。この 2 文字は
 *     \s にマッチしない。上で書いたとおり本物の改行文字（U+000A）自体は \s に含まれて
 *     捕まるので、取りこぼすのは「生テキストとしてスキャンする箇所に、改行がエスケープ
 *     表現のまま埋め込まれている」場合に限る
 *
 * なぜ全部塞がないか：本ツールは warn のみで CI に入れない道具（冒頭コメント参照）。
 * 誤検知（無関係な数字を拾う）を増やすとこの台帳自体の価値が落ちるため、上記は
 * 「意図的に塞がない」設計判断であり、やり残しではない。
 *
 * ⚠️ 最重要：W8（延いては check-world.mjs 全体）が warn を出さないことは、
 * CLAUDE.md §2-13「架空データの正本は data/world/」を守っている証明にはならない。
 * 新しい数値・名前を書くときは、まず data/world/ を見て書くのが本来の手順であり、
