...
| ③ | **ユースケース・シナリオ** | [`docs/dify/usecases`](./docs/dify/usecases/) [`docs/handoff`](./docs/handoff/) [`docs/demo`](./docs/demo/) [`mock/js/data/scenarios`](./mock/js/data/scenarios/) | [`docs/dify/usecases/README.md`](./docs/dify/usecases/README.md) | 全サービスの詳細ユースケース（件数は [`docs/service-map.md`](./docs/service-map.md) を参照）、設計書、デモ進行台本と顧客提示資料（`docs/demo/`）、モックのデモ台本（コードなので `mock/` の下） |
...
| ⑤ | **ポータル** | `portal/`（未作成。PR-N2 で新設） | `portal/README.md` | 社内向けポータル（NocoBase Community）の定義・DDL・架空のデモデータ。実データは 1 バイトも入らない。ルート `npm test` の対象外で、検証は `npm run portal:test` |
