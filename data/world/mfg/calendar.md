...
`dify/kb/KN-01/**` は 2026 年 4〜7 月の出来事として一貫して書かれている（Rev.D 2026-06-25・`PC-L3-2026-014`・`QR-2026-Q2-05`）。
実機デモ（Dify Cloud）は現実の日付で行うため、KB は現実の日付に近い方が不自然さが少ない。台本（kn1）は日付を出さないので矛盾は表に出ない
（`docs/handoff/2026-09-24-script-kb-consistency.md` §5-2。PM 判断待ち §9 Q1。日付を動かす案は版の順序と参照先の年を壊すため、決まるまでこのまま未統一として記録する）。
