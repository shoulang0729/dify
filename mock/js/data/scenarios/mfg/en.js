...
Object.assign(window.SCENARIOS.mfg, {
  en1: { template: 'diff',
    ...
    script: {
      ja: [
        { q: '【比較】K 社要求仕様書 Rev.C（2024-11）⇄ Rev.D（2025-09）', a: '42 ページを比較し、変更 6 か所を抽出しました（結果の表）。影響が大きいのは 2 つ：① §4.2 塗膜厚の下限が 15→20 µm に上がり、現行工程の実力値（18〜24 µm）では下限側が外れるロットが出ます。塗装外注 S 社との条件見直しが必要です。② 付録 B「首件（初品）」の定義が材料ロット切替時にも拡張され、初品検査の回数が増えます。K 社用語は社内呼称に対応づけて表記しています（「限度样件」→「限度見本」など）。' },
      ...
      zh: [
        { q: '【比较】K公司要求规格书 Rev.C（2024-11）⇄ Rev.D（2025-09）', a: '比较了42页，提取出6处变更（见结果表格）。影响大的有2处：① §4.2 涂层厚度下限由15提高到20 µm，按现行工艺实力值（18〜24 µm）会出现下限侧超差的批次，需与涂装外协S公司调整条件。② 附录B「首件」定义扩展到材料批次切换时，首件检验次数增加。K公司术语已对应到公司称谓（如「限度样件」→「限度見本」）。' },
  ...
  en2: { template: 'lookup',
    ...
    script: {
      ja: [
        { q: '材料ロット H2508-17（S 社 SPCC t1.2）', a: '逆引きしました（結果の表）。ロット H2508-17（2.4 t）は 3 生産ロットで使用、製品 2 品番・計 4,750 個に展開されています。出荷先は K 社常熟（1,970 個出荷済）と日本本社経由の K 社日本向け（2,400 個、洋上・9/9 大阪着予定）。当社在庫は完成品 380 個（倉庫 A-12）と未使用材料 0.6 t（M-03）です。**すぐ動くべきは 3 つ**：①未使用材料 0.6 t の隔離 ②在庫 380 個の保留 ③洋上分について本社へ先行連絡（到着前に受入保留の準備ができる）。' },
      ...
      zh: [
        { q: '材料批次 H2508-17（S公司 SPCC t1.2）', a: '已反查（见结果表格）。批次 H2508-17（2.4 t）用于3个生产批次，展开为2个品号共4,750件。出货对象为K公司常熟（已出货1,970件）及经日本总部至K公司日本（2,400件，在途，9/9预计到大阪）。本公司库存为成品380件（仓库 A-12）与未使用材料0.6 t（M-03）。**应立即行动的3件事**：①隔离未使用材料0.6 t ②保留库存380件 ③就在途部分先行通知总部（可在到港前准备来料保留）。' },
  ...
  en3: { template: 'upload',
    ...
    script: {
      ja: [
        { q: '【アップロード】引合図面_K社_RFQ-2025-118_朱書きあり.pdf', a: '図面には K 社の手書きの朱書きが 2 か所ありました。1 か所目「M6 タップ ×2 追加」は読み取れたので条件に入れています。2 か所目（1 ページ右下の余白）は判読できず、**推測で埋めずに空欄のまま「K 社への確認事項」に回しました**。図面そのものには書き込んでいません。\n図面から形状（L 型 195×80）・板厚 t1.5・曲げ 2 か所・M6 タップ ×2・塗装を読み取り、過去図面 1,240 件から類似上位 3 件を出しました（結果の表）。最も近いのは現行の SK-3310-A（87%）ですが、**見積上で重要なのは 3 位の SK-1190**です：外形と板厚がほぼ同一で、2023 年に終了した製品の金型 D-092 が倉庫 T-2 に保管されています。抜き型とリブ成形型の改造流用ができれば、新規金型費（推定 RMB 620,000）を大幅に抑えられる可能性があります。※ 本サービスは構想段階で、類似度は形状特徴の機械的な一致率です。' },
        ...
        { q: 'この 3 件の不具合履歴で、新規品で気を付けるべきことは？', a: '3 件の履歴から、新規品（RFQ-2025-118）で予防すべき不具合は 3 つ：\n① **塗装ブツ**（SK-3310-A、2024・2025）：同じ塗装外注 S 社・同色系なので再発リスクあり。8D-25-0912 の是正処置（脱脂液濃度 2 回/日チェック、フィルター 45 日交換）を新規品の工程条件に最初から組み込む\n② **タップ不通**（SK-2207-B、3 件）：M6 ×2 が共通。原因はいずれもタップ折損後の復旧ロットで、タップ交換基準（現行 500 穴）と折損時の全数通止検査をコントロールプランに明記\n③ **リブ割れ**（SK-1190、2020）：今回はリブなしのため直接は該当しないが、t1.5 の曲げ R3 は割れマージンが小さい。K 社に R3→R4 の図面変更を提案できるか、見積時に確認事項として挙げる\nこれらを見積書の「技術確認事項」欄に転記できます。判読できなかった朱書き 1 か所（1 ページ右下の余白）も、同じ欄に確認事項として入れています。' }
      ],
      zh: [
        { q: '【上传】询价图纸_K公司_RFQ-2025-118_带红笔批注.pdf', a: '图纸上有K公司的2处手写红笔批注。第1处「追加 M6 螺纹×2」可以读取，已计入条件。第2处（第1页右下空白处）无法辨识，**未做推测填补，保持空白并列入「向K公司确认的事项」**。没有在图纸本身上做任何标注。\n从图纸读取形状（L型 195×80）、板厚 t1.5、折弯2处、M6螺纹×2、涂装，在1,240张历史图纸中检出相似前3（见结果表格）。最接近的是现行 SK-3310-A（87%），但**对报价最重要的是第3位 SK-1190**：外形与板厚几乎相同，2023年停产产品的模具 D-092 保管在仓库 T-2。若落料模与筋成形模可改造沿用，有望大幅降低新模具费（估计 RMB 620,000）。※本服务处于构想阶段，相似度为形状特征的机械一致率。' },
