...
Object.assign(window.SCENARIOS.mfg, {
  ...
  lg2: { template: 'upload',
    ...
    script: {
      ja: [
        { q: '【アップロード】議事録_2025-06〜08（84 件）、チャットログ_生産技術_2025Q3.txt、作業標準書_全ライン_一覧.xlsx', a: '文書 84 件とチャット 3,120 行から、同一物を別の名前で呼んでいる語を 6 組検出しました（結果の表）。本社との会議で問題になった「ブラケット」は 4 通りの呼び方があり、うち「L 型支架」は旧品番の形状に由来する呼称なので廃止を推奨します。6 組のうち 4 組は既に用語集に標準があるのに守られていない状態、2 組（#1・#4）は用語集に未登録です。' },
      ...
      zh: [
        { q: '【上传】会议纪要_2025-06〜08（84件）、聊天记录_生产技术_2025Q3.txt、作业标准书_全线_清单.xlsx', a: '从84件文件与3,120行聊天记录中，检出6组同一事物用不同名称的词（见结果表格）。与总部会议上出问题的「支架」有4种叫法，其中「L型支架」源于旧品号的形状，建议废止。6组中4组术语表已有标准但未被遵守，2组（#1・#4）未登记术语表。' },
  ...
  lg3: { template: 'form',
    ...
    script: {
      ja: [
        { q: '【フォーム送信】相手：班長 趙／指示：段取り順の変更・初品 3 個・夜勤も／背景：E-47 多発', a: '口頭指示を 5 項目の中国語指示文に書き下しました（結果）。元の指示に無かった「温めるの基準（40℃、測定方法）」を、設備保全課の PX-200 復旧手順から補って明記しています。誤解されやすいのは「今までと逆」「3 個測る」「連絡」の 3 か所で、それぞれ具体化しました。理解確認の質問 3 問を趙さんに口頭で聞いてください。' },
      ...
      zh: [
        { q: '【提交表单】对象：班长 赵／指示：换型顺序变更・首件3件・夜班同样／背景：E-47频发', a: '已将口头指示改写为5条中文指示（见结果）。原指示中没有的「预热基准（40℃、测量方法）」，从设备保全科的 PX-200 恢复步骤中补充并明确。容易误解的是「与以前相反」「测3件」「联系」3处，均已具体化。请口头向赵询问3道理解确认问题。' },
  ...
  lg4: { template: 'form',
    ...
    script: {
      ja: [
        { q: '【フォーム送信】K 社 林さんの増量依頼メール／+12% まで・残りは 11 月前倒し／正式回答は 9/15 後／中国語', a: '受信メールを日本語で要約し、商務中文の返信案を作成しました（結果）。ポイントは「+20% は無理」を先に言わず、「10 月 26,900 個＋11 月上旬 1,900 個」で**合計 28,800 個は満たす**構成にしたことです。金曜の期限には「初步方案」として応え、正式回答は 9/15 の生産会議後と明記しています。' },
      ...
      zh: [
        { q: '【提交表单】K公司林的增量请求邮件／最多+12%・其余11月提前／正式回复9/15后／中文', a: '已用日文总结来信，并生成商务中文回复草案（见结果）。要点是不先说「+20%做不到」，而是以「10月 26,900件＋11月上旬 1,900件」构成**合计满足 28,800件**。对周五期限以「初步方案」回应，并明确正式回复在9/15生产会议后。' },
