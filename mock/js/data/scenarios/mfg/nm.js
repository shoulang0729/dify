...
Object.assign(window.SCENARIOS.mfg, {
  nm4: { template: 'lookup',
    ...
    script: {
      ja: [
        { q: 'SK-2207-B 在庫・納期', a: '在庫・生産計画・入荷予定を照会しました（結果の表）。来週 9/12 出荷なら追加 **3,000 個は対応可能**です（引当可能在庫 1,720 ＋ 仕掛 1,500 ＋ 来週生産の未引当分 1,200 ＝ 4,420）。前提条件は 2 つ：① 船便の締切（木曜 15:00）に合わせて 9/11 までに梱包完了 ② 通い箱 30 箱の確保。保留品 860 個（K 社クレームで検査中）は計算に入れていません。' },
      ...
      zh: [
        { q: 'SK-2207-B 库存・交期', a: '已查询库存、生产计划与到货预定（见结果表格）。若下周9/12出货，追加**3,000件可以对应**（可用库存1,720＋在制1,500＋下周生产未占用1,200＝4,420）。前提条件2项：①配合船期截单（周四15:00），9/11前完成包装 ②确保30个周转箱。保留品860件（K公司客诉检验中）未计入。' },
  ...
  nm1: { template: 'form',
    ...
    script: {
      ...
      zh: [
        { q: '【提交表单】SK-3310-C／SPCC t1.5／18,000件×36个月／有涂装・模具新开／K公司期望 USD 4.60', a: '已按公司成本表累计（见结果表格）。制造成本 RMB 6.45，含标准利润 RMB 7.42（USD 1.04）。**但有1点务必确认**：现行A品售价 USD 4.20 与本次试算 USD 1.04 相差一个数量级。若涂装费 RMB 2.05 与现行A品相同，A品成本应在 RMB 5.9 左右，与 USD 4.20（RMB 30）不符。A品单价可能是「1套（8件）」的价格。' },
  ...
  nm2: { template: 'upload',
    ...
    script: {
      ja: [
        { q: '【アップロード】見積_S工具_2026年度.pdf、Quotation_TC-Tools_FY2026.xlsx、メール_U刀具_报价.txt', a: '3 社の見積を「RMB／本・税抜・年間 2,400 本」に正規化して比較しました（結果の表）。単価最安は TC-Tools（172.4）ですが輸入で納期 6 週間・前払いが必要です。現行 S 工具の +7.5% 値上げは、理由のタングステン高騰から計算すると +5.4% 程度が妥当で、約 2 ポイント過大です。工具寿命を含めた「加工 1 本あたりコスト」では S 工具 0.886 に対し TC 0.690 ですが、TC は当社実績がなく未検証です。' },
      ...
      zh: [
        { q: '【上传】报价_S刀具_2026年度.pdf、Quotation_TC-Tools_FY2026.xlsx、邮件_U刀具_报价.txt', a: '已将3家报价归一为「RMB/支・不含税・年2,400支」进行比较（见结果表格）。单价最低为 TC-Tools（172.4），但为进口，交期6周且需预付。现行S刀具 +7.5% 的涨幅，按其钨价上涨的理由推算约 +5.4% 为合理，高出约2个百分点。含刀具寿命的「每加工件成本」S刀具0.886、TC 0.690，但TC无本公司实绩，未经验证。' },
  ...
  nm3: { template: 'upload',
    ...
    script: {
      ja: [
        { q: '【アップロード】生産日報_L1-L3_0901-0905.xlsx、品質日報_0901-0905.xlsx、設備稼働記録_0901-0905.csv', a: '15 枚の中国語日報と稼働記録を集計し、日本語の週報を作成しました（結果の表）。合計は計画達成 100.3%、不良率 0.42% で目標をわずかに超過。異常値は 2 つ：L3 の不良率 0.52% は新条件表適用直後（9/1〜2）の 1.1% が原因で、9/3 以降は 0.28% に収束しています。L2 は PX-200 のアラーム停止 55 分と塗装ブツで不良率 0.44% です。' },
      ...
      zh: [
        { q: '【上传】生产日报_L1-L3_0901-0905.xlsx、质量日报_0901-0905.xlsx、设备运行记录_0901-0905.csv', a: '已汇总15张中文日报与运行记录，生成日文周报（结果为中文对照）。合计计划达成100.3%，不良率0.42%略超目标。异常值2项：L3不良率0.52%源于新条件表适用初期（9/1〜2）的1.1%，9/3以后已收敛至0.28%。L2因 PX-200 报警停机55分钟及涂装颗粒，不良率0.44%。' },
  ...
  nm5: { template: 'lookup',
    ...
    script: {
      ja: [
        { q: 'ライン別に、直近 6 か月の残業時間と不良率の関係を見せて', a: '2025 年 3〜8 月の月次データをライン別に集計しました（結果の表）。結論：**残業と不良の相関は L2 だけに強く出ています**（+0.78）。L1・L3 はほぼ無関係で、L3 の 8 月は条件切替の影響です。L2 は残業 30h/人を超えた月（6・7 月）に不良率が 0.5% を超え、不良は夜勤帯の塗装工程に集中しています（夜勤 0.61% vs 昼勤 0.34%）。' },
      ...
      zh: [
        { q: '按线显示近6个月加班时间与不良率的关系', a: '已按线汇总2025年3〜8月的月度数据（见结果表格）。结论：**加班与不良的相关只在L2明显**（+0.78）。L1・L3几乎无关，L3的8月是条件切换影响。L2在加班超30h/人的月份（6・7月）不良率超过0.5%，不良集中在夜班涂装工序（夜班0.61% vs 白班0.34%）。' },
