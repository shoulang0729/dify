...
/* ============================================================
   POUT — SVCS[].place === 'out' のサービスの理由文（v1 は 18 件だったが、業種テンプレート化で
   置き場所の無かった全件に画面ができたため rev4（PR-E）時点では 0 件。設計書
   docs/handoff/2026-09-12-portal-industry-rev4.md §11-2・§11-5）。
   置き場所そのもの（画面 id ／ '*' ／ 'out'）は mock/js/data/catalog.js の SVCS[].place が正本（設計書 §4-3）。
   ここが持つのは「なぜポータルに置かないか」という、ポータルの解説レイヤーの文だけ（§4-3）。
   空オブジェクトのまま残す（定数と verify §17-g の検査を温存し、将来 'out' が復活しても動くように。§11-5）。
   ============================================================ */
const POUT = {};
