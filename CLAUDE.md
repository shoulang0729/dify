...
### 並列実行の安定化（タイムアウト / 並列数チューニング）

- **背景**: マルチモデル比較（例: `apps/multi-model-compare*.yml`）で多数の LLM を
  並列実行すると、遅い/詰まるモデルでワークフロー全体が完了せず「止まって見える」。
- **v1 での切り分け**: ボトルネックは Dify の計算資源ではなく、
  ① **OpenRouter 側**（クレジット残・同時実行/レート上限）、
  ② **Dify Cloud Sandbox** の無料枠制限（実行回数・タイムアウト・並列）。
  → v1 では「OpenRouter にクレジット投入」「並列モデル数を絞る（5→3 等）」で凌ぐ。
- **v2 でやる根本対処**（セルフホストなら自由に設定可）:
  - ワークフローの **並列数・深さ上限**（例: `WORKFLOW_PARALLEL_DEPTH_LIMIT` 等の env）を引き上げ。
  - **ノード / ワークフローのタイムアウト**を延長。遅いモデルが詰まっても全体を止めない。
  - 必要なら LLM ノードに**リトライ / フォールバック**やバッチ分割を入れる。
  - env 名・設定箇所は **移行先の Dify バージョンで要確認**。
