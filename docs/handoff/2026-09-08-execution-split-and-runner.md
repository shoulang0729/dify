...
**2026-09-10 追記**：`console_api.py` へのリフレッシュトークン認証の実装（#201・W4-3）と `op: deploy` の追加（#208・W4-4）により、O1〜O4・O9 の前提の一部が変わった。GitHub Actions **run #16**（2026-09-10、`op: deploy`、`codes: DC-01 DC-02 DC-04 GN-02 GN-05 LG-01 LG-04 NM-03`、`confirm: deploy`、`env: cloud-master`、成功 8／失敗 0）と **run #11**（#195、2026-09-09、`op: inspect`）の実機結果を該当行に反映した。**確認できた範囲だけを書き、確認していないことは「未確認」のまま残す。**

| # | 操作 | 実行しているもの | Mac でしかできない理由 | 種別 | 根拠 |
|---|---|---|---|---|---|
| **O1** | DSL の新規インポート | `cloud_deploy.py`（Console API）／Studio 画面 | コンソール API の認証が **httpOnly セッション Cookie ＋ `X-CSRF-Token`**。`localStorage` に `console_token` は存在せず、`Authorization` ヘッダも送られていない。CSRF ヘッダ無しの `fetch(..., {credentials:'include'})` は **401**（#114 時点の観測）。**2026-09-10 追記**：`console_api.py` にリフレッシュトークン認証（Cookie／CSRF 対応）を実装済み（#201・W4-3）。ブラウザのログイン済みセッションが要るのは**リフレッシュトークンを取得する最初の 1 回だけ**（`dify/DEPLOY.md` §8・§10）で、以後はホストランナーが自動更新しながら使い続ける（#212）。**ただし「アプリがまだ存在しない状態からの新規作成」は 2026-09-10 時点でも実機で確認していない。** `cloud_deploy.py` は名前一致が無ければ新規作成にフォールバックする経路を持つが、`dify/DEPLOY.md` §9 は「新規作成は想定していない（12 本は既に Mac から一度デプロイ済み）」と明記しており、run #16 で流した 8 本もすべて既存アプリへの**上書き**（O2 参照）だった | (a) → **2026-09-10 追記**：初回のリフレッシュトークン取得のみ (a)、以後は `run:runner`。**新規作成の経路自体は未実証のまま** | #114 コメント「N1〜N6 の実機観測」／2026-09-10 追記：`docs/handoff/2026-09-08-cloud-auth-and-w4.md`・`dify/DEPLOY.md` §8〜§10・GitHub Actions run #16（2026-09-10） |
| **O2** | DSL の上書きインポート | アプリ名 →「…」→「DSL をインポート」→「上書きしてインポート」（画面）／`cloud_deploy.py`（`POST /console/api/apps/imports` に `app_id` を渡す） | 同 O1（2026-09-10 時点で認証面は解消済み）。**2026-09-10、GitHub ホストランナーから `op: deploy` を実行し（`codes: DC-01 DC-02 DC-04 GN-02 GN-05 LG-01 LG-04 NM-03`）、8 本すべてが上書き経路で `status=completed-with-warnings` を返してインポートに成功した（run #16、成功 8／失敗 0）。Cloud での実行を確認できた** | (a) → **2026-09-10、run #16 で解消を確認（`run:runner` で完結）** | #114 コメント／`dify/DEPLOY.md` §1-④・§9／**GitHub Actions run #16（2026-09-10）** |
| **O3** | 公開（publish） | `POST /console/api/apps/{app_id}/workflows/publish` | 同 O1。さらに **UI はクライアント側でチェックリストを検証し、未解決だと POST を出さない**（`Checklist has unresolved items`。#114 N3）。**2026-09-10 追記**：run #16 で、KB を持たない 8 本（DC-01・DC-02・DC-04・GN-02・GN-05・LG-01・LG-04・NM-03）について、`op: deploy` の公開ステップ（Console API・UI を介さない）が全件成功した。**ただし確認できたのはここまで。**「サーバ側が UI と同じチェックリスト検証をするか」自体は依然として未確認——KB 付き 4 本（KN-01・KN-02・KN-03・GN-01）は `dataset_ids` が空のまま上書きされると紐づけが消える問題（#209、設計済み・未実装）の歯止めで `op: deploy` が exit 2 で止まるためまだ試しておらず、そもそも「未解決チェックリスト」が発生する状況を作れていない | (a) → **KB を持たないアプリに限り 2026-09-10、run #16 で解消を確認（`run:runner`）。KB 付きアプリは未確認のまま** | #114 コメント N3／**GitHub Actions run #16（2026-09-10）**／`dify/DEPLOY.md` §9「既知の制限」／`docs/handoff/2026-09-09-dataset-ids-in-ci.md`（#209） |
| **O4** | 知識検索ノードへの KB 紐づけ | Studio 画面 | 同 O1（認証面は 2026-09-10 時点で解消済み）。マスタ DSL は `dataset_ids: []` なので **上書きインポートのたびに紐づけが外れる**。draft API による差し替え（`--bind-kb draft`）は C5・C6 が未確認。**2026-09-10 追記**：`dataset_ids` を焼き込んだ DSL であれば最初から紐づくこと自体は、Studio 上の手動検証（V2、W4-2）で真だと確認されている。しかし **CI から dataset id を解決する仕組み（#209）は設計済み・未実装**で、焼き込みに使う `DIFY_DATASET_ID_*` は GitHub の secret として登録されておらず、`op: deploy` は空の `dataset_ids` を検知して **exit 2 で止まる歯止め**が入っているだけ。**KB 紐づけは 2026-09-10 時点でも実質 Studio 画面（Mac）が必要** | (a)（変わらず。#209 の実装待ち） | #114 追記／`cloud_deploy.py` docstring／**`docs/handoff/2026-09-09-dataset-ids-in-ci.md`（#209）**／`dify/DEPLOY.md` §9「既知の制限」 |
...
| **O9** | app id / dataset id の採取 | 画面 URL・API 応答／`cloud_deploy.py` の名前一致解決（app id） | 実機に問い合わせないと存在しない値（＝「事実の生成」）。O1・O5 の副産物。**2026-09-10 追記**：**app id 側は解消。** `cloud_deploy.py` は `GET /console/api/apps` から名前一致で app id を自動解決しており、run #16 のログにも `app_id=4f5205f8…` のように出力されている（Mac 不要・`run:runner` で完結）。さらに `op: inspect`（#195、run #11、2026-09-09）で、下書きから Rerank 関連 6 項目（`reranking_enable`／`provider`／`model`／`reranking_mode`／`top_k`／`score_threshold`）を読み取り専用で採取できることも実証済み。**dataset id 側は未解消。** Datasets API から名前引きする設計（#209）はあるが**未実装**で、現状は `${DIFY_DATASET_ID_*}` を人が Secrets に置く前提のまま | app id: (a)(c) → **2026-09-10、(c) に解消を確認（`run:runner`）**／dataset id: **(a)(c) のまま（#209 の実装待ち）** | `dify/env/README.md`「`apps:`」節／**GitHub Actions run #16（2026-09-10）・run #11（#195、2026-09-09）**／`docs/handoff/2026-09-09-dataset-ids-in-ci.md`（#209） |
