...
- **設計書**：`docs/handoff/2026-09-11-sysops-usecase.md`（**rev1.3**＝§15-2 で PR-4 の矛盾 4 件を決着。**PM 判断 9 件すべて確定**＝§15-1。5 軸判定表のタグ軸は rev1.1 で `catalog.js` から機械的に埋め直し済み）
...
> **rev1.3（2026-09-11）で PR-4 の設計内の矛盾 4 件に決着をつけた（設計書 §15-2）。**
> **A**：スコープ「全社」は**台帳 9 行すべて**を出し、**重大障害（重要度 高 × 障害発生中）を先頭に固定して赤で強調**する。
> 「重大障害だけ」は CIO・CEO ロールの**既定フィルタ**（状態＝障害発生中）に読み替え、モックの既定は「状態（すべて）」。
> スコープに 4 つ目は足さない。ペルソナ岸本 奈津に対する確定は **全社 9 行（SYS-01〜09）／自分が使う 3 行（SYS-07・08・09）／
> 担当 1 行（SYS-07）**。
> **B**：**AC-18 を正**とし、§6-9 の `day` は「**全件 `normal`**（ふつうの朝）」と読む。`degraded` は `night` に置く
> （`day` 緑 9／`night` 赤 1・黄 2・灰 3・緑 3／`holiday` 灰 7・緑 2）。
> **C**：`<script src>` は #267（`demo.js`）の後なので **38 本**（AC-12 の「37 本」を訂正。判定の主は verify §17-a の PASS）。
> **D**：行内 AI ボタン（`PCTXDEF.sys`・`prowAi`）は **PR-4 の範囲外**。**PR-5 で台本と一緒に**配線する。
> **§10 の件数と id 一覧・AC-17・AC-19〜AC-23 は変わらない**（新トークン 0・色の直値 0 も維持）。

- [ ] 共通業務の 2 番目に「システム稼働状況」（3 言語）。`portal.html` の `<script src>` は **38 本**（`sys.js` は `back.js` の後ろ）
- [ ] 状態が 7 種類出る。`offhours`／`batch` は**保存されていない**（導出）
- [ ] スコープ「全社」で **9 行**。時刻プリセット（平日 10:20／平日 22:40／休日 03:10）で**同じ 9 行の状態だけ**が変わる（`holiday` で灰 7 件）
- [ ] スコープを切り替えると **9 → 3 → 1 行**。ホームの「いま止まっているシステム」は `day` で 0 件・`night` で 3 件
