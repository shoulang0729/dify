...
## §18 追補（2026-09-13・PR-B #301 の reviewer 申し送り）

### 18-0. この節の位置づけ

**§0〜§17 は 1 バイトも書き換えていない**（`CLAUDE.md` §4 の作法。決着は追補として足す。
`2026-09-11-sysops-usecase.md` §15 と同じ形）。以下の「決定」は、**当該箇所について §14-1 の表より優先する**。
記録の出所は **PR-B（#301）の reviewer 申し送り**で、実装は #301 でマージ済みである。
...
| PR-C（`cust` → 取引先） | 未マージ（本節を書いた時点で open な PR は無い） | — | — |
| PR-D（新画面 4 枚の中身） | 未マージ（同上） | — | — |
| PR-E（`place` 27 件・規則 1 撤回） | 未着手 | — | — |
| PR-F（fallback・規則 5/6 の文言） | 未着手 | — | — |

本文の数字の基準は `d484ff7`（§0 冒頭）のままで変えていない。

---

### 18-1. 決定 1 —— §14-1 の「17-a 改訂不要」は**誤りだった**

**誤りの所在**：§14-1 の表の 17-a 行に「**改訂不要。**新しいデータファイルを作らない（既存 7 本に定数を足すだけ）」
と書いた。**「新しいデータファイルを作らない」までは正しいが、「既に存在するデータファイルを新たに読む」を見落としていた。**

**なぜ改訂が要るか**：§5-3 の `renderIdentity()` が `#whoName ← PL(FEED[ind].persona.name)` ・
`#whoRole ← PL(FEED[ind].persona.role)` と指示している。`FEED` の正本は `mock/js/data/home.js` であり
（§1-3 に「`FEED[].persona` は**読むだけ**」と明記した）、**`mock/portal.html` はそのファイルを読み込んでいなかった**。
さらに §14-1 の **17-o** 自身が「`mock/js/data/home.js` の `FEED[業種].persona` が 3 業種とも存在する」を要求しており、
verify がポータル経路で `FEED` を読めなければこの検査は成立しない。**17-a・17-o・§5-3 の 3 つが互いに矛盾していた。**

**決定 18-1a（`mock/portal.html`）**：`<script src="js/data/home.js"></script>` を
**`js/data/catalog.js` の後・`js/data/style.js` の前**に足す。位置は `CLAUDE.md` §2-3 の読み込み順
（`data/ui → data/catalog → data/home → data/style → data/scenarios/* → …`）と**同じ並びに揃えるため**であり、
`catalog.html` とポータルで並びが食い違わないようにする。**`home.js` の中身は 1 バイトも変えない**（§1-3 は維持）。

**決定 18-1b（`tools/verify.mjs` §17-a）**：期待並び（`expectedOrderP`）に `'js/data/home.js'` を
**3 番目**として足す。17-a の検査そのもの（順・本数・実在・相対パス）は変えない。
§14-1 の表の 17-a 行は、次のように読み替える。

> **17-a ｜ 改訂（PR-B）。**期待並びに `js/data/home.js` を 3 番目として足す（`data/ui → data/catalog → data/home → data/style`）。
> 検査の内容（順・本数・実在・相対パス）は変えない。

**決定 18-1c（`tools/lib/load.mjs`）**：`PORTAL_DATA_KEYS` に **`FEED`** を足す
（verify が vm で読む定数の一覧。ここに無い定数は §17 のどの検査にも掛からない）。
§14-1 **17-e** に書いた `PORTAL_ONLY_KEYS` とは**別のリスト**である点に注意（下の 18-2 も同じ）。

**§1-2 の読み替え**：「変更する範囲」に **`mock/portal.html`** と **`tools/lib/load.mjs`** を加える。
`mock/portal.html` は §15-1 の PR-F 行（`.mockbar` の `indNote`）で既に触る前提になっており、
§1-2 の列挙から落ちていたのは書き漏らしである。**§1-3 の「触らない範囲」は 1 行も緩めない。**

**受け入れ条件への影響**：新しい AC は足さない。18-1a〜18-1c は
**AC-16**（業種チップで会社名・部門名・拠点・ログイン中の人が替わる）を成立させるための前提であり、
**AC-41**（verify / regress / `npm test` が PASS）に含まれる。

---

### 18-2. 決定 2 —— `PORTAL_DATA_KEYS` に `PSYS`/`PSYSEV`/`PSYSST`/`PSYSNOW` が漏れていた（**既存バグ**）

**事実**：`mock/js/data/portal/sys.js` は sysops-usecase の PR-4（#272・`eb51127`）で新設されたが、
**`tools/lib/load.mjs` の `PORTAL_DATA_KEYS` に 4 定数（`PSYS` / `PSYSEV` / `PSYSST` / `PSYSNOW`）が載っていなかった**。
そのため #272 以降、**verify §17 のポータル系の検査が `sys` 画面の行データを一切見ていなかった**
（§17-e の 3 言語検査も掛かっていない）。**rev4 が作った問題ではない。**

**なぜ rev4 の PR で直したか**：§14-1 の **17-n**（業種別の行データ定数は、その業種で見える画面の分のキーを持つ）は
**`PSYS.mfg` を読めなければ実装できない**（§10 の製造テンプレート＝設備 6 件が検査対象）。
別 Issue に切り出すと 17-n が実装できないため、**#301 で合わせて足した**。

**決定 18-2a**：`PORTAL_DATA_KEYS` に `PSYS` / `PSYSEV` / `PSYSST` / `PSYSNOW` を足す（#301 で実施済み）。
副作用として、**既存の `sys` 4 定数に §17-e（`{ja,zh,en}` の 3 言語・空でない・`en` にかな残りなし）が新たに掛かる**。
#301 の `node tools/verify.mjs` PASS で、既存データがこの検査を通ることを確認済み。

**決定 18-2b（PR-C・PR-D の implementer への申し送り）**：
**`mock/js/data/portal/**` に定数を足したら、`tools/lib/load.mjs` の `PORTAL_DATA_KEYS` を同時に更新する。**
rev4 で新設する定数は次のとおりで、**どれも 2 つのリストに入れる**（片方だけだと検査が静かに素通りする）。

| PR | 新設する定数 | `PORTAL_DATA_KEYS`（`tools/lib/load.mjs`） | `PORTAL_ONLY_KEYS`（§14-1 17-e） |
|---|---|---|---|
| PR-C | `PPART`（← `PCUST` 改名） | **`PCUST` を外して `PPART` を入れる** | 同左（§14-1 17-e のとおり） |
| PR-D | `PQUAL` / `PORDER` / `PCRED` / `PREG` | **4 つとも足す** | 4 つとも足す（§14-1 17-e のとおり） |

**§16-4・§16-5 の読み替え**：**AC-20〜AC-23・AC-24〜AC-28 の検査は、`PORTAL_DATA_KEYS` に新定数が載っていて
初めて機械で効く。**reviewer は PR-C・PR-D の diff で `tools/lib/load.mjs` の更新を必ず確認する
（**更新が無いのに verify が PASS するのは「検査が掛かっていない」ためであり、PASS を根拠にしない**）。

---

### 18-3. PR-B 時点の中間状態（**設計どおり**。不具合として扱わない）

**何が起きているか**：`cust`（取引先）の行データ `PQTR` / `PCUST` は **PR-C で業種化する**と決めてあるため
（§8・§15-1）、PR-B（#301）をマージした時点では**まだ 1 つの世界の値しか持っていない**。
結果として、**業種チップで製造・金融に切り替えても `cust` 画面には IT 世界の取引先の社名が残る**。

**判断**：**設計どおりの中間状態**であり、PR-B の受け入れ条件（AC-12〜AC-19）はどれも `cust` を対象にしていない。
verify §17-p（世界の混ざりの検出）は **`{mfg,fin,it}` の形をした定数の `.mfg` / `.fin` の中だけ**を見るので、
業種化前の `PQTR` / `PCUST` は検査対象に入らず **FAIL しない**（検出漏れではなく、**適用範囲どおり**）。

**いつ解消するか**：**PR-C**。§16-4 の **AC-20〜AC-23** が解消の確認そのものである
（3 業種で行数が 製造 3 / 金融 5 / IT 4、担当者 6 / 5 / 6、四半期表の見出しと単位が業種で替わる、
IT の見え方は `origin/main` と同じ）。**PR-C がマージされるまで、reviewer と PM はこの見え方を不具合として起票しない。**

**PM への共有**：PR-B だけを Pages で見ると「製造チップなのに取引先が IT 世界のまま」に見える。
**デモに出すなら PR-C までをまとめてから**にする。

---

### 18-4. この追補で変えていないもの

- **§0〜§17 の本文**（`c2d7a4e` / #298 のまま）。18-1・18-2 は §14-1 の表の**読み替え**として書いた
- **受け入れ条件の本数**（46 件のまま。追加も削除もしていない）
- **`CLAUDE.md`・`.claude/**`**（§13-4 の「改定は不要」は依然として有効。
  §2-3 の読み込み順は 18-1a が**従った**規則であって、変えていない）
- **`mock/js/data/home.js` の中身**（§1-3 のとおり読むだけ）
- **`localStorage` のキー**（`mock.lang` / `mock.theme` の 2 つのまま。AC-19 は PR-B で確認済み）

---

### 18-5. 決定 3 —— AC-22 は §8-1 の IT リテラルより**優先する**

**矛盾の所在**：`origin/main`（`10ef527`）の `V.cust`（`mock/js/portal/render.js`）の顧客表は

- ヘッダ：`顧客` / `拠点` / `受注後` / `受注前` / **`受注残`**（`{t:'受注残',n:1}`。**単位は列名に入っていない**）
- 行の AI：`prowAi(['rs3', 'rs1'], { scr: 'cust', id: c.id })`

これに対し §8-1 に書いた `PPART.it` は `head` の 5 列目が `受注残（百万円）`、`ai` が `['gn8']` で、
**main と機能差分がある**（列名に単位が増え、行の AI が 2 本 → 1 本に減る）。
PR-C（#304）は §8-1 のリテラルどおり実装したため、**AC-22（IT の `cust` は改名以外の差分ゼロ）と矛盾した**。

**決定 18-5a**：**AC-22 を優先する。`PPART.it` の `head` と `ai` は main と同じ値に戻す。**

```js
it: {
  head: ['顧客', '拠点', '受注後', '受注前', '受注残'],   // ← 「（百万円）」を付けない
  calc: 'deals',
  ai: ['rs3', 'rs1'],                                     // ← main の prowAi と同じ 2 本
  rows: [ …4 行は §8-1 のまま… ]
}
```

**理由**：PM 方針「**IT のフロントは残す**」＝ PR-C は IT の見え方を**回帰させない**。
列名への単位追加も行 AI の 2 本 → 1 本も回帰にあたる。
`mfg` / `fin` の `head` / `ai` は §8-1 のままでよい（新規なので回帰の対象が無い）。

**§8-1 の読み替え（該当 2 か所）**：

> `it.head: ['顧客', '拠点', '受注後', '受注前', '受注残（百万円）']`
> → **`it.head: ['顧客', '拠点', '受注後', '受注前', '受注残']`**
>
> `it.ai: ['gn8']` → **`it.ai: ['rs3', 'rs1']`**

**手書きの `ai` は暫定である**：行の AI は **PR-E** で `SVCS[].place` から導出し直す（§11-3・規則 1'）。
§8-1 の `ai` は **PR-C 時点の暫定値**にすぎず、最終形を決めるのは PR-E である。
したがって PR-C の段階で IT の `ai` を減らす実益は無い（減らした分が PR-E で戻る）。
`mfg` / `fin` の `ai` も同じ性格で、**PR-E が §11-3 の表に合わせて見直してよい**。

**実施**：#304 の差し戻し対応で実施する。reviewer の確認は AC-22 のとおり
（IT 業種で `cust` を開き、列名・行・数字・注記が main と一致すること。差分は「顧客 → 取引先」の**改名だけ**）。

---

### 18-6. 決定 4 —— AC-26 は PR-D の範囲では満たせない。**PR-E へ移す**

**事実**：`mock/js/portal/app.js` の `pctxRow(scr, id)` は 3 分岐しか持たない。
...
function pctxRow(scr, id) {
  ...
  return null;            // ← qual / order / cred / reg はここに落ちる
}
```

**`PCTXDEF` に 4 キーを足しても（§9-5）、`pctxRow` が行を返さない限り文脈カードは出ない。**
実行ドロワーは「行なし」の汎用表示（`PT.ctxNoRow`）に落ちる。壊れはしないが、AC-26 は満たされない。

**原因**：**§15-1 が PR-D の触るファイルに `mock/js/portal/app.js` を入れていなかった**
（`mock/js/data/portal/{front,svc,ui}.js` と `mock/js/portal/render.js` だけ）。**設計書の見落としであり、
implementer の逸脱ではない。**

**決定 18-6a**：**AC-26 を PR-D（§16-5）から PR-E（§16-6）へ移す。**
PR-E は §15-1 で既に `mock/js/portal/app.js` を触るので、**PR の境界も触るファイル集合も変えずに済む**。
PR-E で `pctxRow` に 4 分岐を足す。形は implementer が決めてよく、**条件は 2 つだけ**：

1. 行は **`PCTXDEF[scr]` のキー順**で `PQUAL` / `PORDER` / `PCRED` / `PREG` の `rows[i]`（配列）と 1:1 に対応させる
2. **業種限定の定数なので `pd()` を使わず直接引く**（18-8）

```js
/* 参考（PR-E。実装はこの形でなくてよい） */
const PCTXSRC = { qual: () => PQUAL, order: () => PORDER, cred: () => PCRED, reg: () => PREG };
if (PCTXSRC[scr]) {
  const d = PCTXSRC[scr]()[pstate.ind];            // 業種限定。it フォールバックを掛けない
  const row = d && d.rows.find(r => r[0] === id);
  if (!row) return null;
  return PCTXDEF[scr].reduce((o, k, i) => (o[k] = row[i], o), {});
}
```

**PR-D 側の扱い**：行内 AI ボタンの配線（`prowAi` に `{ scr, id }` を渡す）と `PCTXDEF` の 4 キー追加は
**PR-D で入れてよい**（PR-E の前提になる）。PR-D の時点で文脈カードが「行なし」表示になるのは
**中間状態として許容する**（18-3 と同じ扱い。**reviewer と PM は不具合として起票しない**）。
#303 はこの旨をコード上のコメントで明記しており、**そのまま残してよい**。

**§16-5 の読み替え**：

> **AC-26** は **PR-D の受け入れ条件から外す**。PR-D が確認するのは
> 「4 画面の行に AI ボタンが出て、`data-ctx-scr` / `data-ctx-id` を持ち、押すとドロワーが開く」まで。

**§16-6 の読み替え**：

> PR-E の受け入れ条件に **AC-26**（`qual` / `order` / `cred` / `reg` の行から AI を押すと、
> `PCTXDEF` の列が **3 言語のラベル付き**で文脈カードに出る）を加える。

**受け入れ条件の本数は 46 件のまま**（移動であって追加ではない）。
**PR-D の触るファイルに `app.js` を加えない**（加えると PR-E と衝突する。§15-1 は変えない）。

---

### 18-7. 確認 —— 17-g（`ai` の id 実在検査）は **PR-C で実装する**

§14-1 の **17-g** 行は担当 PR を「PR-C / PR-D」と併記していた。**PR-C（#304）の差し戻し対応で、
1 ブロックにまとめて実装する。**

- **検査対象**：`PPART[ind].ai` ・ `PPART[ind].rows[].ai`、および将来の
  `PQUAL` / `PORDER` / `PCRED` / `PREG` の `[].ai`（`SVCS` に id が実在すること）
- **定数が存在しなければ skip する書き方にする**（`typeof` で見て無ければ飛ばす）。
  PR-C の時点では `PQUAL` 等がまだ無いため、skip が無いと FAIL する
- **PR-D 側は 17-g を触らない。** PR-D が `tools/verify.mjs` に足すのは 17-e・17-q への追随と、
  `tools/lib/load.mjs` の `PORTAL_DATA_KEYS`（18-2b）だけ

**理由**：`tools/verify.mjs` を触る PR は直列（§15-1）。同じブロックを 2 本の PR が別々に足すと必ず衝突するので、
**先にマージされる側（PR-C）が 1 回で書く**。

---

### 18-8. 注記 —— **業種限定の定数は `pd()` を使わず直接引く**

**事実**：`V.qual` / `V.order` / `V.cred` / `V.reg` の先頭にある
...
という guard は**正当な実装**であり、#303 の設計逸脱ではない。

**理由**：§4-1 の `pd()` は `obj[pstate.ind] ?? obj.it` と **`it` にフォールバックする**。
これは **3 業種すべてが値を持つ定数**（§7-1 の「業種化する —— 23 定数」）のための契約であって、
**1 業種しか持たない定数**（§7-1 の「新設（1 業種だけが持つ）—— 4 定数」＝
`PQUAL` / `PORDER` は `mfg` のみ、`PCRED` / `PREG` は `fin` のみ）では成立しない。
`pd(PQUAL)` を金融で呼ぶと `PQUAL.it` が無いので `pd()` の既定 `[]` になり、`.rows` を読んだ時点で落ちる
（あるいは逆向きの定数では**別世界の行が他業種の画面に出る**＝§17-p が狙っている事故そのもの）。

さらに **`renderCanvas()` は 20 画面すべてを業種にかかわらず毎回描く**（表示は `hidden` 属性で切り替える。§4-3）。
つまり **見えない業種でも `V.qual` は必ず呼ばれる**ので、**guard は必須**である。

**§4-1 の `pd()` の契約に足す注記**（§7-1 の「新設（1 業種だけが持つ）—— 4 定数」に掛かる）：

> **業種限定の定数は `pd()` を使わず直接引く。**`pd()` の `?? obj.it` フォールバックは
> 3 業種すべてが値を持つ定数のためのものであり、1 業種だけの定数に使うと
> 他業種の画面に別世界の行が出るか、既定の `[]` を配列として読んで落ちる。
> 直接引いたうえで、**値が無ければ空の器を返す guard を描画関数の先頭に置く**
> （`renderCanvas()` は見えない画面も毎回描くため。§4-3）。

**`PSYS` 系についての観察（今回は直さない）**：`sys` の描画は PR-B（#301）で `pd(PSYSEV)` の形になっており、
金融では `it` にフォールバックする。**`sys` は金融で `hidden` なので画面には出ず、§17-p も
`.fin` キーの中だけを見る（`PSYSEV` に `fin` キーが無い）ため FAIL しない。**
**現状のままでよい**が、将来 `sys` を金融でも出すことにしたら、この 1 か所を上の注記の形（直接引き＋guard）に直す。

---

### 18-9. 進捗の更新（**18-0 の表を本節で置き換える**）

18-0 の本文は書き換えない（`CLAUDE.md` §4 の作法）。**進捗表の最新は本節**である。
...
| PR-C（`cust` → 取引先） | **差し戻し対応中**（18-5 の `PPART.it` 戻し ＋ 18-7 の 17-g） | — | **#304** |
| PR-D（新画面 4 枚の中身） | **条件付き可**（18-6 のとおり AC-26 を PR-E へ移す。**PR-C の後にマージ**し、`tools/verify.mjs` は rebase） | — | **#303** |
| PR-E（`place` 27 件・規則 1 撤回・`pctxRow` 4 分岐・**AC-26**） | 未着手 | — | — |
| PR-F（fallback・規則 5/6 の文言） | 未着手 | — | — |

**マージ順**：**#304 → #303**（両方が `tools/verify.mjs` を触るため。§15-1 の「verify を触る PR は全部直列」）。

本文の数字の基準は `d484ff7`（§0 冒頭）のままで変えていない。
**受け入れ条件の本数も 46 件のまま**（18-6 は PR 間の移動であって増減ではない）。

---

### 18-10. この追補（第 2 版）で変えていないもの

- **§0〜§17 の本文**（`c2d7a4e` / #298 のまま）。18-5〜18-8 は §8-1・§16-5・§16-6・§14-1・§4-1 の**読み替え**として書いた
- **既存の 18-0〜18-4**（`10ef527` / #302 のまま。進捗表の差し替えは 18-9 に置いた）
