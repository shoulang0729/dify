...
- **変更前**：`は次の **4 区分**を置くリポジトリ：` … `（デモ投入用の入力サンプル）。地図はトップ `README.md`、`
- **変更後**：`**4 区分**` を `**5 区分**` に。`（デモ投入用の入力サンプル）` の直後、`。地図はトップ` の前に次の 1 文を挿入する：
...
- **108 行 変更前**：`- **何を**：会社（青嶺精工／青岭精工／Seirei Seiko Co., Ltd.）・拠点（蘇州工場・Japan HQ）・人・部署・…`
- **108 行 変更後**（先頭だけ差し替え。以降はそのまま）：
...
- **110 行 変更前**：`…1 行ずつ載せる（現状 10 件）`
- **110 行 変更後**：`…1 行ずつ載せる（**現状 12 件**：製造 10／金融 1／IT 1）`
...
- **変更前**：`| `run:mac` | PM の Mac（ブラウザのログイン済みセッションが要る） |`
- **変更後**：`| `run:mac` | PM の Mac（**PM の手元でしか動かせないもの**：ブラウザのログイン済みセッション／ローカル docker） |`
...
- **159 行 変更前**：`P3 候補：`?v=` キャッシュスタンプの機械検証、`tools/bundle.mjs`（単一ファイル生成）、`scenarios/` の 1 サービス 1 ファイル化。`
- **159 行 変更後**：
...
- **160 行（`top.html` の扱い）変更前**：`②③ が `catalog.html` に入ったので **削除（PR-3）**。`
- **160 行 変更後**：`②③ が `catalog.html` に入ったので **削除済み**（`mock/` に存在しない）。`
