...
### ② 人物 5 名のうち 4 名の改名（1 名は**完全衝突**）

ポータルのモックで使っている 5 名を既存 37 名（mfg 20・fin 17）と突き合わせた結果：

| ポータルの名前 | 判定 | 推奨 |
|---|---|---|
| **高橋 亮** | **✗ 完全衝突。`data/world/mfg/people.csv` に `takahashi-ryo,高橋 亮,Ryo Takahashi` が既にいる** | **藤井 亮 / 藤井亮 / Ryo Fujii**（姓のみ変更＝差し替え最小） |
| 李 婷 | ⚠ 名「婷」が既に 2 人（呉 婷・潘 婷） | 李 雯 / 李雯 / Li Wen |
| 周 建国 | ⚠ 名「建国」が既に 2 人（馮 建国・楊 建国） | 周 建偉 / 周建伟 / Zhou Jianwei |
| 村井 拓也 | ✓ 問題なし | そのまま |
| 石田 真希 | ⚠ 姓「石田」が `fin` にいる（石田 由美） | 島田 真希 / 岛田真希 / Maki Shimada |

**IT 世界は 3 つの世界の登場人物が同じ画面に出る唯一の世界**なので、「建国さん」「婷さん」が 3 人ずついるとデモの口頭説明で実害が出る。
