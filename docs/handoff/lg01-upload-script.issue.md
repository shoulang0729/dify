...
## PM 判断の確定（2026-09-12。上の「PM 判断待ち」は決着済み）

**P1 は案 B（`SVCS.lg1.desc` は据え置き）／P2 は採用（1 ターン目に一般訳 `技术通知书` を出し 2 ターン目で人が直す演出）／P3 は別 Issue 予定（本件では起票しない）** で確定した。P1 を案 B にした理由は、**提供中（`st: 1`）の 12 本は「実機で動く」ことが売りであり、実機で出来ないこと（体裁保持）を顧客に見える `desc` に書かない**ため。これにより **`mock/js/data/catalog.js` は対象外**となり、上の「やること」2 は削除、PR-1 が触るのは **`mock/js/data/scenarios/mfg/lg.js` / `docs/service-map.md` / `docs/dify/usecases/LG-01.md` の 3 ファイル**、受け入れ条件 9 は「`git diff --stat` が **3 ファイルだけ**（`catalog.js` が現れたら FAIL）」に読み替える。**§4-3 の台本全文・§6 の `usecases/LG-01.md` §10 追記・`npm run index` の再生成・`regress` 差分ゼロ（`--update` 禁止）は変わらない。** 詳細は設計書 `docs/handoff/2026-09-12-lg01-upload-script.md` **§14 追補**（§14-2 の読み替え表が本文より優先する）。
