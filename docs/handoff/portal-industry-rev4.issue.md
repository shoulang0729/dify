...
**進捗（2026-09-13 第 3 版）**：PR-C **#304**（`1dd3240`）・PR-D **#303**（`dade3b7`）**マージ済み**、PR-E **#306** はレビュー中。追補 **§18-11〜18-16** で 3 件を決めた。① **AC-26 の「3 言語のラベル」は PR-F**（`demo.js` の `dCtxRows()` が `PCTXDEF` のキー名を生ラベルに使っており、`PT.ctx*` を引いていない既存挙動。`sys` も同じ）—— `PCTXLBL`（画面 → キー → `PT` キー名）を `demo.js` に新設し、`PT` に **6 キー追加**（`sys` 5 ＋ `cred` 1）、verify **17-r** を足す（**AC-47**）。18-6a の「`PCTXDEF` のキー順で `rows[i]` と 1:1」は**誤り**で、#306 の `PCTXTABLES`（列 index の宣言）が正しい（18-11c）。② **`V.home` が業種に関係なく IT の `PDEALS` を描く見落とし**を **PR-G（新設・M・`render.js` の `V.home` のみ）**で直す —— 製造＝品質/受注/設備、金融＝与信/当局通達、IT は現状維持。**既存定数から描くだけで新データは足さない**（**AC-48〜AC-50**、Playwright **P-9 / P-10**）。③ `V.goal` の「この画面の AI」欠落を #306 が `paiScreenBlock('goal')` で補ったのは**可**（AC-31・AC-32 の確認に必要）。**受け入れ条件は 46 → 50 件、PR は 7 → 8 本。マージ順は #306 →（PR-F ／ PR-G は並列可）。**PR-F の確定範囲は **§18-14**、進捗表の最新は **§18-15**。
