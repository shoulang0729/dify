...
PM 確定事項（2026-09-10）：**用途は「顧客提示デモ」と「実運用の社内システム」の両方**／**設計は実データを見ながら行う**／**リポジトリは分離、かつ public**。

**⚠️ 新リポは public。リポジトリに入るのは「定義」と「架空のデモデータ」だけで、実データは 1 バイトも入らない。** 本番は**空の定義をデプロイして中身を別途投入**する。これは `dify/apps/*.yml`（マスタ）＋ `dify/env/<env>/env.yml`（`${VAR}` で外から）と**同じ作法**（`CLAUDE.md` §2-12）。public にする目的は**顧客と社内に見せること**。

## 決めたこと（設計書 §0）

| 問い | 結論 |
|---|---|
| リポジトリ | **分ける。新リポ `shoulang0729/portal`（public）。** 現リポからは 1 バイトも移さない |
| 分ける理由（**入れ替わった**） | **決定的なのは R1 ランタイム境界**：現リポは `dependencies` ゼロで `npm test` が `node tools/*.mjs` 2 本／NocoBase は Koa 常駐 ＋ Sequelize ＋ Redis ＋ cron ＋ docker。**「public に実データを置けない」は取り下げ**（実データが入らないので成り立たない）。取り下げた事実は設計書 §1-2b に**消さずに残した** |
| リポジトリに入るもの／入らないもの | 入る＝**定義**（画面・スキーマ・ロール・WF）＋**架空のデモデータ**＋設計書。**入らない＝実データ・秘密・接続先の実値**（すべて `${VAR}`） |
...
| 最初に作る 1 つ | **P0「私のページ」**（3 画面・SSO 無し・チャット UI を作らず Dify WebApp を iframe）。**主目的は AC-1／AC-4 ＝ 定義を移送できるかの実機確認** |
| **public にする以上やること（新規設計）** | ① **`tools/check-nodata.mjs`** — 実データが入らないことを **`check-world.mjs` と同じ allowlist 方式**で機械検査。構造 G1〜G5 は **FAIL**、中身 N1〜N6 は「**何を拾って何を拾わないか**」を §7-4-3 に明記（N4 メール・N5 電話・N6 実在ドメインは FAIL／N1 人名・N2 社員番号・N3 部署名は warn ＋ baseline）② **公開されるものの一覧 P-1〜P-12**（**PM の判断が要るのは P-3 業務ルールの閾値と P-8 所定労働時間・締め日の 2 つだけ。どちらも `${VAR}` に逃がせば判断不要**）③ **秘密の置き場は `dify/env` ＋ `env.example` の型をそのまま持ち込める**（§7-6） |
...
PR-1（本 PR・先行必須）
  ...
  └── PR-3   docs/dify/** README.md CLAUDE.md ┘ PM 判断は了承済みなので着手できる
```
...
- **`.github/workflows/**`** — 特に **`pages.yml` の `path: mock`**（`CLAUDE.md` §2-8）。**NocoBase は常駐サーバ ＋ DB なので、そもそも GitHub Pages に載らない**（public / private 以前の問題として、Pages は論点にならない）
...
## PM 判断（8 件中 7 件は了承済み。**残り 1 件が回答待ち**。設計書 §11）

| # | 論点 | architect の推奨 |
|---|---|---|
| 11-1 ✅ | **V-PM-1 をやるか**（上の「PM に 1 つだけお願い」） | **やる。P0 着手前** |
| 11-2 ✅ | `PC-19 お知らせ・全社掲示ストア` を採番してよいか | **採番する**（`platform-components.md` は「ID と名称は固定」と宣言しているため PM 承認が要件だった。了承済み） |
| **11-3** ⏳ **回答待ち** | Dify の `user` に社員 ID を素通しするか、仮名にするか | **仮名 ＋ ポータル側で逆引き**（PC-09 の Langfuse トレースにも残るため。PC-02 の記述に触れるので PM 判断） |
| 11-4 ✅ | NocoBase の AI 機能（`plugin-ai`）を使わない、を確定してよいか | **使わない。** AI の正本は Dify（67 サービスの資産がすべて Dify 側にある） |
| 11-5 ✅ | 新リポの名前と可視性 | `shoulang0729/portal`（**public**） |
| 11-6 ✅ | §9-1・§9-2 の追記を PR-3 で適用してよいか | **適用**（PC-19 の新設を含む） |
