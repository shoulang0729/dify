...
| | 内容 |
|---|---|
| **§10 ① の状態** | **⏳ PR-1 着手前 → ✅ 決定済み**（本決定）。**② も決定済み**（§11-2）。§10 の PM 判断待ちで残るのは **③④⑦⑧** |
...
**PR の状態は §11-2 を見ること**（本節を書いた時点で architect が「PR-1 は着手可／PR-3 は止まっている」と
誤って書いたが、**実際は PR-1〜PR-5 がすべてマージ済み**だった）。

### 11-2. PR-1〜PR-5 はすべてマージ済み（**2026-09-11 時点の事実の記録**）

> **本節は判断ではなく、`origin/main` を見て確かめた事実の記録である。**
> §8（PR 分割）・§10（PM 判断待ち）の本文は**書き換えない**。状態だけをここに書く。

`git log --oneline origin/main | grep it-industry` の結果：

| PR | 内容 | commit | 状態 |
|---|---|---|---|
| **PR-1** | 業種 `it` の足場と `data/world/it/`（翠雲システムズの世界） | `e6701d8`（**#255**） | **✅ マージ済み** |
| **PR-2** | 両業種サービス 11 件を IT 業に開放し `HOME.it` / `FEED.it` を新設 | `019c325`（**#258**） | **✅ マージ済み** |
| **PR-3** | **新分類 `sl`（営業・案件管理）と IT 固有サービス 10 件** | `4911894`（**#259**） | **✅ マージ済み** |
| **PR-4** | IT 固有 10 件の台本（`scenarios/it/`）と `check-world` の IT バケット | `eca4d5c`（**#261**） | **✅ マージ済み** |
| **PR-5** | IT 固有 10 件の `place` 付与と、未採番の名刺 OCR 候補（`PNEW`）の GN-08 への統合 | `f0a7667`（**#264**） | **✅ マージ済み** |

**したがって §10 ②（分類 `sl` の新設と 10 件の配置）は、PM 承認のうえ #259 で実装済み。**
**下の 10 件の管理番号は採番が確定し、`CLAUDE.md` §2-11 により永久欠番になった**（削除しても再利用しない）：

| 管理番号 | 名称（ja） | `cat` | `place` |
|---|---|---|---|
| **SL-01** | 引合の受注確度推定 | `sl` | `proj` |
| **SL-02** | 失注理由の蓄積と傾向分析 | `sl` | `proj` |
| **SL-03** | 過去提案の横断検索と再利用 | `sl` | `proj` |
| **KN-09** | 取込文書の分類自動振り分け | `kn` | `know` |
| **KN-10** | 規程と現場運用の食い違い検出 | `kn` | `know` |
| **PO-05** | 年休の取り残し検知と取得計画 | `po` | `ppl` |
| **PO-06** | 残業の偏りからの要員リスク検知 | `po` | `ppl` |
| **PO-07** | AI利用実績からの削減時間の見積 | `po` | `kpi` |
| **GN-08** | 名刺の読み取りと項目抽出 | `gn` | `cust` |
| **DC-10** | 予実差の理由の書き起こし | `dc` | `kpi` |

**main の現況**（`npm test` の regress 出力。`4a4f3d1` 時点）：**15 分類・35 中分類・81 サービス**、
業種別 **mfg 49 ／ fin 29 ／ it 25**（2 業種以上 11）。IT の 25 件は
**両業種 11 ＋ IT 固有 10 ＋ SO-01〜04（`docs/handoff/2026-09-11-sysops-usecase.md` の分類 `so`）4** の内訳。
**§5 の「変更前後の件数と id 一覧」は PR-1〜PR-5 時点の値であり、その後 `so` の 4 件が別設計で足されている。**

**本決定（3-②）で新たに着手可能になるものは無い。**社名は PR-1 の時点で案 A（翠雲システムズ）のまま実装されており、
**PM 確認はそれを追認したもの**である。**差し替えは発生しないので、§3-4-3 が挙げた 3 か所
（`data/world/it/company.md`・`INDUSTRIES.it.wordmark`・IT 台本）を直す作業も発生しない。**
