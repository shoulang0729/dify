...
- **2026-09-08 追記**：§6 に、カタログが 43→67 件に広がる過程で本表（33 件）の対象外だった 26 件の実現性突合を追記した（`docs/handoff/2026-09-08-implementation-order-v2.md` §5-4 PM-4 の決定）。**ファイル名・上のタイトルの「33」はそのまま残す**（Issue・PR・`docs/dify/README.md` からのリンクが load-bearing）。§0〜§5（33 件時点の判定・本文）は 1 件も変更していない
...
## 6. 実現性突合（追加 26 件：2026-09-07 以降にカタログへ加わったサービス）

> **本書の対象は §0〜§5 が 2026-09-06 時点の 33 件、本節が 2026-09-07 以降に加わった 26 件。ファイル名・本文タイトルの「33」は実態（現在 67 件中の 59 件分＝ 33 ＋本節 26）と合っていないが、Issue・PR・`docs/dify/README.md` からのリンクが load-bearing のため改名しない**（`docs/handoff/2026-09-08-implementation-order-v2.md` §5-4 PM-4 の決定）。
>
> 対象：`docs/handoff/2026-09-08-implementation-order-v2.md` §1-1 で機械的に確認された、§2（33 件）の範囲外の 26 件。カタログは現在 67 件で、33 件（本書 §2）＋パートナー 8 件（`docs/handoff/2026-09-06-partner-usecases.md` §5、PT-01〜08）＝ 41 件がこれまでの突合済み範囲、残り **26 件**が本節の対象：
> `CV-01 CV-02 CV-03 CV-04 DC-08 DC-09 EG-01 FA-01 FA-02 FA-03 FA-04 FA-05 GN-06 GN-07 KN-06 KN-07 KN-08 PO-01 PO-02 PO-03 PO-04 RS-01 RS-02 RS-03 RS-04 RS-05`（41 ＋ 26 ＝ 67。`mock/js/data/catalog.js` の `SVCS` と一致）。
>
> **判定は新規に作らない。** ◎○△ の値は `docs/dify/usecases/README.md`「一覧」の実現性列（正本）から機械的に転記した。ここに書くのは「なぜその判定なのか」の根拠で、各 `docs/dify/usecases/<管理番号>.md` の **§4（Dify 構成）・§10（未確定・リスク）を実際に 26 件すべて読んで**要約したもの。列構成・凡例は §1-2・§2 に合わせる。**「台本」「st 設計→意見」の 2 列は 33 件当時の個別レビュー作業（台本の文言修正・成熟度の異議）の記録であり、26 件にはこの作業自体が存在しない（台本は `usecases/*.md` 執筆時に整合済み、成熟度は `docs/handoff/2026-09-08-finance-catalog.md` で確定済み）ため省く**。

### 6-1. 突合表（26 行）

| id | サービス | テンプレ | 定番パターン | アプリ種別 | 実現性 | 判定の根拠（§4・§10 の要約） | 参照 |
|---|---|---|---|---|---|---|---|
| CV-01 | 提案・ピッチ資料の作成（候補先選定・比較企業分析） | form | データ照会＋レポート生成 | Workflow | ○ | 検索ツール・Outline 参照・LLM の骨子組み立てのみで標準機能の範囲。候補先探索の精度が公開情報の質に依存する点、過去の類似案件アーカイブの整備状況が未確認の点で ◎ にしていない | `usecases/CV-01.md` §4・§10 |
| CV-02 | 面談前ブリーフの作成 | form | データ照会＋要約 | Workflow | ○ | HTTP リクエストと軽量 LLM の要約のみで標準機能。**未実装の 2 依存**（`KN-07` の接触履歴検索、`GN-06` の `tasks` API への `client_id` 列追加）の解消が前提のため ◎ にしていない | `usecases/CV-02.md` §4・§10 |
| CV-03 | 審査コメントのドラフト作成 | upload | 文書QA＋レポート生成 | Workflow | ○ | ファイル抽出＋ドラフト生成は標準機能。過去の審査コメントアーカイブ（Outline）の整備状況と、コメントの「良し悪し」評価がモデル差・人手評価に依存する点で ◎ にしていない | `usecases/CV-03.md` §4・§10 |
| CV-04 | KYCスクリーニングとエスカレーション整理 | upload | 文書QA＋分類 | Workflow | ○ | 抽出・KB 照合・エスカレーション判定は標準機能。個人情報（実質的支配者情報）の**越境可否の法務確認**と KYC 審査基準の Outline 整備状況が前提のため ◎ にしていない。**RS/CV/FA で唯一 `PC-10` の強化適用を要する**（§6-2 CV-04 参照） | `usecases/CV-04.md` §4・§10 |
| DC-08 | 報告レビュー（提出前チェック／受領後の論点整理） | form | 文書QA＋レポート生成 | Workflow | ○ | 2 段 LLM＋Code（集計・突合・検算）は標準機能。(1) 報告規定が種別ごとには文書化されていない、(2) `PC-17`（指摘・回答台帳）の新設が要る、(3) 論点の質の評価がモデル差で揺れる、の 3 点で ◎ にしていない | `usecases/DC-08.md` §4・§10 |
| DC-09 | 議案・報告書・提案書のドラフト作成（テンプレート選択） | form | レポート生成＋分類（決裁ルート判定） | Workflow | ○ | フォーム→ Code（決裁ルート表を JSON 定数で判定）→ LLM の型で標準機能。統合元の 2 サービス（旧＃4・＃5）とも標準機能で成立するため ○ | `usecases/DC-09.md` §4・§10 |
| EG-01 | 上流工程の仕様支援（読解・質問回答・エラー対処） | qa | 文書QA(RAG)＋差分比較 | Chatflow | ○ | 章単位 KB＋`PC-12`（章ペア突合への転用）は標準機能。仕様書・エラーコード表の KB 化運用と、文脈依存の矛盾検出の検出力が評価セット待ちの点で ◎ にしていない | `usecases/EG-01.md` §4・§10 |
| FA-01 | GL勘定のリコンシリエーション | upload | 差分比較＋データ照会 | Workflow | ○ | xlsx 抽出＋Code 突合（唯一の数値計算の出所）は標準機能。GL・補助簿のフォーマットが会計システムにより異なりうる点、前月結果の保存粒度が未確定な点で ◎ にしていない | `usecases/FA-01.md` §4・§10 |
| FA-02 | 月次クローズの実行と報告 | form | データ照会＋レポート生成 | Workflow | ○ | フォーム＋ Code（進捗率・営業日計算）＋ LLM ドラフトは標準機能。チェックリストマスタの Outline 整備と `FA-01` との自動連携経路が未確定な点で ◎ にしていない | `usecases/FA-02.md` §4・§10 |
| FA-03 | 財務諸表のレビュー（整合性・監査対応） | upload | 文書QA＋差分比較 | Workflow | ○ | xlsx/docx 抽出＋ Code 表間突合＋ KB 照合は標準機能。会計方針マスタの整備状況と、監査で聞かれやすい点の粒度が監査法人により異なりうる点で ◎ にしていない | `usecases/FA-03.md` §4・§10 |
| FA-04 | 財務モデルの作成と決算反映 | upload | データ照会＋レポート生成 | Workflow | △ | 抽出・差分計算自体は標準機能で単発分析なら ○ 相当。ただし**「モデル」という継続的な成果物の永続化（モデルストア）が本サービス固有・最優先で新規に要り、版管理・確定フローも未設計**のため △ に落とす（§6-2 参照） | `usecases/FA-04.md` §4・§10 |
| FA-05 | バリュエーションのレビュー | upload | 文書QA＋分類 | Workflow | ○ | xlsx 抽出＋ KB 照合＋修正案の提示は標準機能。社内レビュー基準の Outline 整備状況と、`FA-04`／`CV-01` との入出力スキーマ整合が未確定な点で ◎ にしていない | `usecases/FA-05.md` §4・§10 |
| GN-06 | 頼まれ事・放置業務の追跡 | form | データ照会＋分類 | Chatflow | ○ | Question Classifier＋ Code（日付解決・営業日計算・集計）＋ HTTP リクエストは標準機能で **Dify 側は易しい**。**`tasks` テーブルと API（`PC-01` の隣。最優先の別出し）が本体**で、放置日数・完了率・相談判定の価値の 8 割はテーブル側にあるため ◎ にしていない | `usecases/GN-06.md` §4・§10 |
| GN-07 | 幹部来訪・出張のアテンド段取り | form | データ照会＋レポート生成 | Chatflow | △ | 事実抽出→ Code 計算→予定表組立の構成自体は標準的だが、**`PC-18`（出張案件ストア。新規・本件が主要な書き手）と ICS レンダラ・配布記録が本体**で Dify アプリだけでは成立しない（§6-2 参照） | `usecases/GN-07.md` §4・§10 |
| KN-06 | 事務手続の照会 | qa | 文書QA(RAG) | Chatflow | ◎ | KB 1 本＋ 2 段 LLM の定型 QA。`KN-01`・`KN-02` と同型で、依存 PC は `PC-03`・`PC-05` の 2 つのみ（26 件中最も浅い）。W4 着手の先頭候補（§6-3 参照） | `usecases/KN-06.md` §4・§10 |
| KN-07 | 行内営業情報の検索（日誌・接触履歴） | qa | 文書QA(RAG) | Chatflow | ○ | KB 検索＋時系列整理は標準機能。営業日誌・面談記録が**自由記述の docx のままだと `client`／`rep_id`／`date` のメタデータ付与が手作業になる**点、`PC-02` の閲覧範囲設計が単純な部署帯では足りない可能性がある点で ◎ にしていない | `usecases/KN-07.md` §4・§10 |
| KN-08 | 当局通達・ガイドラインDB（照会・過去比較） | diff | 差分比較＋文書QA | Chatflow | ○ | MinerU＋`PC-12`（KN-05 と共用）＋ 2 KB 検索は標準機能。**過去の照会回答をどこまで遡って登録できるか**（遡及登録範囲）と通達登録の当番運用が未確定な点で ◎ にしていない。W4 着手候補（§6-3 参照） | `usecases/KN-08.md` §4・§10 |
| PO-01 | アンケート・インタビュー収集 | qa | データ照会＋分類 | Chatflow | ○ | Question Classifier＋曖昧判定＋回答ストア（専用 PC 番号は割り当てない）で標準機能。**複数回答者・複数設問の並行実施には回答ストアが要る**（1 回のチャットで完結する少人数向けなら現状でも動く）点で ◎ にしていない | `usecases/PO-01.md` §4・§10 |
| PO-02 | アイデアの募集・蓄積・投票集計 | form | 文書QA＋分類 | Workflow | ○ | KB 照合（類似度判定）＋統合提案＋投票集計は標準機能。提案アーカイブ（Outline）の初期投入と、投票ツールの実体（フォーム集計か専用システムか）が未確定な点で ◎ にしていない | `usecases/PO-02.md` §4・§10 |
| PO-03 | 小テスト・コンプライアンスチェックの実施と集計 | form | データ照会＋分類 | Workflow＋Chatflow | ○ | 出題＝ Workflow、受験＝ Chatflow（`Workflow as Tool` 検討中）の 2 段構成、採点は Code で標準機能。受験セッションストア（`quiz_sessions`／`quiz_answers`。専用 PC 番号なし）の新設と、受験側 Chatflow の実装方式の確定待ちで ◎ にしていない | `usecases/PO-03.md` §4・§10 |
| PO-04 | 稼働の集計とコスト配分の提案 | upload | データ照会 | Workflow | ○ | xlsx 抽出＋ Code 按分計算＋ KB 参照は標準機能。**`PC-04`（工数・案件管理システム）の対象一覧に無い**ため第 1 段階は手動アップロード止まりな点、配分ルールの構造化が未了な点で ◎ にしていない | `usecases/PO-04.md` §4・§10 |
| RS-01 | 企業・業界ニュースの自動収集と配信 | form | データ照会＋要約 | Workflow | ○ | 検索ツール＋ Code（重複除去・期間絞込）＋ LLM 要約は標準機能。**監視設定ストア（`rs1_watchlist`。本サービス固有・最優先）が新規に要る**点、中国語ニュースに対する検索ツールの精度・到達性が未検証な点で ◎ にしていない。W4 着手候補（§6-3 参照） | `usecases/RS-01.md` §4・§10 |
| RS-02 | セクター・発行体のモニタリング | form | 文書QA＋差分比較 | Workflow | ○ | 検索ツール＋ Outline 前回結果取得＋ Code 突合は標準機能。前回結果を残す粒度（セクター単位かセクター×期か）と、`RS-01` との検索基盤の共用可否が未確定な点で ◎ にしていない | `usecases/RS-02.md` §4・§10 |
| RS-03 | 顧客IR・決算の収集と日本語要約・比較 | upload | 文書QA＋データ照会 | Workflow | ◎ | PDF 抽出＋ Code 比較表組み立ては、既存の `DC-01`・`GN-05` と同型のパターンで Dify 標準機能と `tools/dify_extractor` だけで十分にまかなえる。26 件で最も条件の揃った ◎。W4 着手候補（§6-3 参照） | `usecases/RS-03.md` §4・§10 |
| RS-04 | 市場・企業データの照会（金融情報端末・契約データベース） | lookup | データ照会 | Workflow | △ | 照会→整形→解説という Dify 側のロジック自体は簡単だが、**外部の契約データ源（金融情報端末・契約データベース）への API 接続が前提**で `PC-04` の対象システム一覧に含まれていないため △ に落とす（§6-2 参照） | `usecases/RS-04.md` §4・§10 |
| RS-05 | ダッシュボード出力からの気づき分析 | upload | データ照会＋要約 | Workflow | ○ | PDF/CSV 抽出＋ Code 計算（差分・外れ値検出）＋ LLM 言語化のみで完結し、**外部システム依存ゼロ**（§8「特になし」）。26 件で最も依存の浅い構想だが、BI ダッシュボード出力形式が顧客ごとに異なりうる点と、因果関係を断定しない書き方のプロンプト調整が要る点で ◎ にしていない | `usecases/RS-05.md` §4・§10 |

### 6-2. △ 3 件（RS-04・FA-04・GN-07）の詳細

既存 33 件の △ 6 件（nm1 nm4 nm5 en2 en3 gn4）はいずれも「照会先システムの API が本体」という同じ理由だった。26 件の △ 3 件はそれぞれ理由が異なる。

**RS-04 市場・企業データの照会（金融情報端末・契約データベース）**
- Dify 側の構成（`Parameter Extractor` → `HTTP Request` → `Code` 整形 → `LLM` 解説）自体は `nm4`（在庫・納期照会）と同じ「lookup 型」で難しくない
- 引っかかるのは **`platform-components.md` PC-04「依存する外部システム」欄に金融情報端末・契約データベースが定義されていない**こと（現行は QMS／BPMS／稟議 WF／ERP／MES／WMS／加工貿易手冊台帳／カレンダーのみ）。`RS-04.md` §8・§10 は「architect 側で PC-04 の対象システム一覧に追記するか、別出し先を検討する必要がある」と明記しており、本 PR の対象外（PR-2 の範囲、`docs/handoff/2026-09-08-implementation-order-v2.md` §6-2 (3)）
- さらに (1) 契約先データ源が API を提供しているか、(2) 中国拠点からの到達性、(3) データ利用許諾の範囲（再配布・保存の可否）の 3 点が未確認（`RS-04.md` §10）。確認が取れるまでは「担当者が端末から手で取った数値を貼って整形」に落とす第 1 段階で成立する

**FA-04 財務モデルの作成と決算反映**
- ファイルアップロード・抽出・差分計算は `RS-03`・`FA-01` と同じ標準機能の範囲で、単発の決算反映だけなら ○ 相当（`FA-04.md` §10 に明記）
- △ に落ちているのは「モデル」という**継続的な成果物**の性質そのもの：**モデルストア（与信先ごとの前提・実績・変更履歴。本サービス固有・最優先）が新規に要り、かつ版管理・確定フローが未設計**。他の RS/FA が「都度実行して都度返す」型なのに対し、FA-04 は「前提を積み上げて保守し続ける」型であるため、実装の性質が一段重い
- モデルストアが無い間は「新規作成」モードでの単発分析のみ動き、継続的な保守・履歴管理ができない（`FA-04.md` §8）

**GN-07 幹部来訪・出張のアテンド段取り**
- 依頼された事実の抽出（LLM）→ 既存事実とのマージ・矛盾検出（Code）→ 基礎データ照合（Knowledge Retrieval）→ 予定表組立（LLM）という流れ自体は他の Chatflow 型サービスと変わらない
- △ の理由は **`PC-18`（出張案件ストア）が新規部品で、本サービスが主要な書き手**であること。`visit_cases`／`visit_facts`／`visit_itineraries`／`visit_distributions` の永続化、版と差分の管理、配布記録（誰に何版をどの言語・チャネルで送ったか）、ICS ファイルの生成（LLM に書かせずレンダラで組み立てる `METHOD:PUBLISH`・`VTIMEZONE`・UID 固定・SEQUENCE 加算）が本体で、**「Dify のアプリだけでは成立しない」と `GN-07.md` §8 が明記**している唯一の RS/CV/FA/GN 系サービス
- 段階導入（`GN-07.md` §8）：**W1** は PC-18 なしで 1 案件 1 会話・Markdown 表返却のみ（モックのデモはここに相当）、**W2** で PC-18 と PC-13（PDF・ICS）、**W3** で PC-04（メール・カレンダー・社用車予約の取り込み）と PC-11。**この W1〜W3 は `docs/handoff/2026-09-08-implementation-order-v2.md` の全体波 W1〜W7 とは別物**（`GN-07.md` 内部の段階呼称。設計書 X-3・PM-6 で「段階 1/2/3」への改称が推奨されている。本 PR では `GN-07.md` を変更しないため呼称はそのまま）

### 6-3. W4（金融第一陣）5 件の詳細（KN-06・KN-08・DC-09・RS-01・RS-03）

`docs/handoff/2026-09-08-implementation-order-v2.md` §3-1 の W4 は「試行版・金融第一陣」で、26 件のうち最初に着手する波。5 件とも `docs/handoff/2026-09-08-finance-catalog.md` §2-2 の優先度 ★ 6 件のうちの 5 件（★ 6 件目の RS-04 は PC-04 拡張が前提のため W5）。

| id | 実現性 | 依存 PC（固有） | W4 着手の根拠 |
|---|---|---|---|
| **KN-06** 事務手続の照会 | ◎ | PC-03 PC-05 | 26 件で唯一 ◎ が 2 つある片方。依存 PC が 2 つのみで、W2・W3 までに立つ部品だけで足りる（新規 PC 0）。事務手続書・様式を条番号単位で KB 化すれば T01〜T05 は主要商品（運転資金貸出・為替予約）だけでも動く（`KN-06.md` §8） |
| **KN-08** 当局通達・ガイドラインDB | ○ | PC-03 PC-05 **PC-12**（KN-05 と共用） | `PC-12` は W3 で立つため新規 PC は不要。`kb-notices` に新旧 2 版を手動登録すれば比較・照会の主要テストは動く（`kb-inquiry-history` が空でも「見つかりません」で定型対応、`KN-08.md` §8）。未確定は通達登録の当番運用と過去照会の遡及登録範囲（運用の詰めであり実装のブロッカーではない） |
| **DC-09** 議案・報告書・提案書のドラフト作成 | ○ | PC-01（`due`）PC-05 PC-13 | 依存 PC はすべて W3 までに立つ。決裁ルート判定は Code の定数（規程 → JSON）で PC を要しない。フォーム→ Markdown ドラフト＋要記入一覧までは KB・BPMS 連携なしでも動く（`DC-09.md` §8） |
| **RS-01** 企業・業界ニュースの自動収集と配信 | ○ | PC-11 PC-13 PC-14 | 依存 PC はすべて W3 までに立つ。**監視設定ストア（`rs1_watchlist`）は第 1 段階を `tools/jiandaoyun`／`tools/microsoft_excel_365` の表で代替できる**ため、専用テーブルの新設を待たずに着手できる（`RS-01.md` §8）。中国語ニュースの検索ツール精度は W4 着手時に評価する項目として残る |
| **RS-03** 顧客IR・決算の収集と日本語要約・比較 | ◎ | PC-03 PC-06 PC-13 | 26 件で唯一 ◎ が 2 つあるもう片方。既存の `DC-01`・`GN-05` と同型（ファイルアップロード→抽出→ LLM 要約）で新規性が最も低い。**過去期抽出結果ストアが無くても今期の要約のみは動く**（比較ができないだけ、`RS-03.md` §8） |

**W4 に △ は 1 件も無い**（RS-04 は ★ だが PC-04 拡張が前提のため W5）。`docs/handoff/2026-09-08-implementation-order-v2.md` §5-4 のとおり、突合の無い 26 件の中で最もリスクが低く、金融で「今日作れる（◎）」の 2 件（KN-06・RS-03）が両方含まれる波。

### 6-4. CV-04（実質的支配者の個人情報・越境の法務確認）

CV-04（KYC スクリーニングとエスカレーション整理）は、**実質的支配者の個人情報を扱う唯一の RS/CV/FA サービス**（`CV-04.md` §8）。他の RS/CV/FA サービスは氏名・出資比率程度までしか個人情報を扱わないか、そもそも扱わないのに対し、CV-04 は KYC の性質上、実質的支配者の氏名・出資比率に加え、住所・連絡先・身分証番号を含む提出書類（登記情報・実質的支配者申告書）を直接受け取る。

- 実装上は `Code` ノードでマスクスニペットを適用し（身分証番号・電話番号・住所を `[ID]` 等に置換）、LLM には出力への転記も禁じる二重の防御を置くが（`CV-04.md` §4・§3）、**越境（マスク後とはいえ中国側 SiliconFlow に個人情報を渡してよいか）の法務確認が実装より先**（`CV-04.md` §10）
- NG の場合は `models/ollama` 等の自前モデルへの切替が必要（`PC-08`・`PC-10` と同じ論点、`docs/dify/platform-components.md` の既存定義の範囲内）
- **これは PC 番号の新設や判定の変更を要する話ではなく、実装着手前に必ず解いておくべき法務論点**という位置づけで、実現性評価は他の CV 系と同じ ○ のまま（Dify 側の構成自体に技術的な難所は無いため）

### 6-5. 検算

- 26 件の内訳：**◎ 2（KN-06・RS-03）／○ 21／△ 3（RS-04・FA-04・GN-07）／× 0**。`docs/dify/usecases/README.md`「一覧」の実現性列から 26 件分を機械的に数え直した値と一致し、`docs/handoff/2026-09-08-implementation-order-v2.md` §5-4 の想定（◎2/○21/△3/×0）とも一致する
- 26 ＋ 41（既存突合済み：本書 §2 の 33 ＋ `2026-09-06-partner-usecases.md` §5 の PT 8）＝ **67**。`mock/js/data/catalog.js` の `SVCS.length` と一致
- 本書 §0〜§5（33 件時点）の判定・本文は本節の追記により**一切変更していない**（`git diff` で確認可能）
